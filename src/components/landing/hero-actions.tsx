            ...
            {modalOpen === 'CREATE' && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 w-full max-w-md relative shadow-2xl">
                        ...
                        <h2 className="text-2xl font-black italic text-white mb-1">SOLICITAR CAMPEONATO</h2>
                        <p className="text-sm text-gray-400 mb-6">Envie os dados para nossa equipe liberar seu acesso.</p>
                        ...
                        <form action={async (formData) => {
                            ...
                            <div>
                                <label className="text-xs text-gray-500 uppercase font-bold">Nome do Campeonato</label>
                                <input name="name" placeholder="Ex: Copa dos Amigos" className="w-full bg-[#0f0f0f] border border-white/20 rounded p-3 text-white focus:border-emerald-500 outline-none mt-1" required />
                            </div>

                            <div>
                                <label className="text-xs text-gray-500 uppercase font-bold">Liga Base</label>
                                <div className="relative">
                                    <select name="leagueType" defaultValue="" className="w-full bg-[#0f0f0f] border border-white/20 rounded p-3 text-white focus:border-emerald-500 outline-none mt-1 appearance-none cursor-pointer">
                            ...
                            <div>
                                <label className="text-xs text-gray-500 uppercase font-bold">Seu WhatsApp</label>
                                <input name="whatsapp" placeholder="(11) 99999-9999" className="w-full bg-[#0f0f0f] border border-white/20 rounded p-3 text-white focus:border-emerald-500 outline-none mt-1" required />
                            </div>

                            <button disabled={loading} className="w-full bg-gradient-to-r from-emerald-500 to-lime-500 hover:from-emerald-400 hover:to-lime-400 text-black font-black py-3 rounded mt-2 uppercase tracking-wide disabled:opacity-50 transition-all shadow-lg hover:shadow-emerald-500/20">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 w-full max-w-md relative shadow-2xl">
                        ...
                        <div className="text-center mb-6">
                            ...
                            <h2 className="text-2xl font-black italic text-white">ENTRAR NA LIGA</h2>
                            <p className="text-sm text-gray-400">Digite o código (Slug) da liga que você recebeu.</p>
                        </div>
                        ...
                        <form action={async (formData) => {
                            ...
                            <div>
                                <input
                                    ...
                                    className="w-full bg-[#0f0f0f] border border-white/20 rounded p-4 text-center text-white text-lg font-bold focus:border-emerald-500 outline-none placeholder:font-normal placeholder:text-gray-600 uppercase"
                                    autoFocus
                                    required
                                />
                            </div>

                            <button disabled={loading} className="w-full bg-white text-black hover:bg-gray-200 font-bold py-3 rounded mt-2 uppercase tracking-wide disabled:opacity-50">
