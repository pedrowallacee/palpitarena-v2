...
import { Calendar, ChevronRight, CheckCircle2, Trophy, Lock, Edit3, Eye, Trash2 } from "lucide-react"
...
interface RoundProps {
    round: {
        id: string
        name: string
        status: string // OPEN, CLOSED, FINISHED, SCHEDULED
        deadline: Date | string
        matches: any[]
    }
    slug: string // Slug do campeonato
    hasPredicted?: boolean // Se o usuário já palpitou
    canManage?: boolean // Se é admin (para poder deletar)
}

export function RoundListItem({ round, slug, hasPredicted, canManage }: RoundProps) {
    const [isOpen, setIsOpen] = useState(false)
    const [isDeleting, setIsDeleting] = useState(false)

    // Verifica status da rodada (Lógica de tempo e status)
    ...
    // Filtra palpites feitos
    const predictedMatches = round.matches?.filter((m: any) => m.predictions && m.predictions.length > 0) || []
    const showPredictions = predictedMatches.length > 0
        ...
        <div className={`group relative bg-[#1a1a1a] border rounded-xl overflow-hidden transition-all hover:bg-[#202020] ${statusConfig.border}`}>
            ...
            >
                ...
                <div className="flex items-start gap-4 flex-1">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            {/* Badge de Status (Se não for Aberta) */}
                            {!isOpenForBets && (
                ...
                <div className="flex items-center gap-2 w-full md:w-auto mt-2 md:mt-0">
                    ...
                    {/* Botão de Expandir Palpites (Se tiver palpites) */}
                    {showPredictions && (
                        <button
                        ...
                        >
                            {isOpen ? 'Ocultar' : 'Meus Palpites'}
                            <ChevronRight className={`w-3 h-3 transition-transform duration-300 ${isOpen ? 'rotate-90' : ''}`} />
                        </button>
                    )}

                    {/* BOTÃO PRINCIPAL: PALPITAR ou CARTELA */}
                    <Link
                        href={`/campeonatos/${slug}/rodada/${round.id}`}
                        onClick={(e) => e.stopPropagation()}
                        className={`
                            flex-1 md:flex-none h-10 px-6 rounded-lg flex items-center justify-center gap-2 font-black uppercase text-xs tracking-widest transition-all
                            ${isOpenForBets
                            ? "bg-emerald-500 hover:bg-emerald-400 text-black shadow-[0_0_15px_rgba(16,185,129,0.2)] hover:shadow-[0_0_25px_rgba(16,185,129,0.4)] hover:-translate-y-0.5"
                                ...
                                <div key={match.id} className={`flex flex-wrap md:flex-nowrap items-center justify-between p-2 rounded-lg border transition-all gap-2 ${bgClass} ${resultBorder}`}>
                                    ...
                                    <div className="flex items-center gap-3 w-full md:w-auto justify-center md:justify-start mb-2 md:mb-0 flex-1 md:flex-none">
                                        <div className="flex flex-col items-center w-10">
                                            {match.homeLogo ? <img src={match.homeLogo} className="w-6 h-6 object-contain" alt="" /> : <div className="w-6 h-6 bg-gray-800 rounded-full"/>}
                                        ...
                                        <div className="flex flex-col items-center w-10">
                                            {match.awayLogo ? <img src={match.awayLogo} className="w-6 h-6 object-contain" alt="" /> : <div className="w-6 h-6 bg-gray-800 rounded-full"/>}
                                    ...
                                    <div className="w-auto md:w-[15%] flex justify-end pl-2 border-l border-white/5 md:border-none min-w-[50px]">
                                        {prediction.isProcessed ? (
                                            <div className="flex flex-col items-end">
                                                <span className={`text-xl font-black font-teko leading-none ${pointsText}`}>+{prediction.pointsEarned}</span>
